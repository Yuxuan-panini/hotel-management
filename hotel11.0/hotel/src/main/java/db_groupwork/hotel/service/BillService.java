...
public class BillService {
    ...
    public boolean makeOrder(int customerId, int hotelId, Date checkin, Date checkout, String roomType){
        ...
        if((orderMapper.isHaveRoomType(hotelId,roomType)>0)&&(orderMapper.getRestRoom(hotelId,roomType))>0){
            ...
            int cost=Room.fee.get(roomType)*differentDays;
            if(orderMapper.getBalance(customerId)>cost){
                orderMapper.purchase(customerId,cost);
                orderMapper.genBill(new Bill(customerId,hotelId,new Timestamp(System.currentTimeMillis()),in,out,differentDays,roomType));
                return true;
            }
        }
        return false;
    ...
    public boolean cancel(int bill_id, int customer_id){
        if(unsubscribeMapper.isDeleteable(bill_id,customer_id)==1){
            ...
            unsubscribeMapper.backMoney(customer_id,(int)b.getBillFee());
