<template>
  <div>
    ...
    <el-card>
      <el-row :gutter="20">
        <el-col :span="16">
          <div class="oo">总报表</div>
        </el-col>
        <el-col :span="8"></el-col>
      </el-row>
      <el-table :data="areareportList" style="width: 100%">
...
export default {
  ...
  created () {
    this.getareareportList()
  },
  methods: {
    async getareareportList () {
      const result = await this.$http.post('getareareportop', {'token': window.sessionStorage.getItem('token')})
      const res = result.data
      if (res.code === 0) {
        return this.$message.error('获取信息失败,请重新登陆')
      }
      this.areareportList = res.areareportList
    },
    async showdetaildialog (hotelmsg) {
      const result = await this.$http.post('getareahotelreportop', {'token': window.sessionStorage.getItem('token'), 'area': hotelmsg.area})
      const res = result.data
      if (res.code === 0) {
        return this.$message.error('获取信息失败,请重新登陆')
      }
      this.reportList = res.areahotellist
