<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>入住查询</el-breadcrumb-item>
    </el-breadcrumb>
    <el-card>
      <el-row :gutter="20">
        <el-col :span="16">
      ...
      <el-table :data="orderList" style="width: 100%">
        ...
        <el-table-column label="操作" width="180">
          <template slot-scope="scope">
            <el-button @click="confirmcheckin(scope.row.id)" type ="primary">确认入住</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
...
export default {
  ...
  methods: {
    async getorderList () {
      const result = await this.$http.post('getcheckinmsg', {'token': window.sessionStorage.getItem('token'), 'consumer_id': this.consumer_id})
    ...
    async confirmcheckin (orderid) {
      const result = await this.$http.post('checkin', {'token': window.sessionStorage.getItem('token'), 'order_id': orderid})
      const res = result.data
      if (res.code === 0) {
        return this.$message.error('入住失败请与管理员联系')
      }
      location.reload()
      return this.$message.success('入住成功')
    }
  }
}
